...
struct VenueAnalyticsView: View {
        ...
        NavigationStack {
            ScrollView {
                VStack(spacing: 16) {
                    ...
                    // Only show rating for the selected period — never the all-time store average.
                    if let average = viewModel.averageRatingInPeriod {
                        ratingCard(average: average, count: viewModel.ratingCountInPeriod)
                    }
    ...
    private func ratingCard(average: Double, count: Int) -> some View {
        AnalyticsSectionCard(title: L(.venueCustomerRating)) {
            HStack(spacing: 10) {
                ...
                VStack(alignment: .leading, spacing: 2) {
                    Text(String(format: "%.1f", average))
                        .font(.title2.bold())
                    Text(String(format: L(.venueRatingsCount), count))
                        .font(.caption)
