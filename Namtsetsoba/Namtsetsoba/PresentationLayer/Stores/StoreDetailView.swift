...
struct StoreDetailView: View {
    ...
    var body: some View {
        ...
        .task { await viewModel.loadBaskets(storeId: store.id) }
        .refreshable { await viewModel.loadBaskets(storeId: store.id) }
    }
    ...
    private var basketsSection: some View {
        VStack(alignment: .leading, spacing: 12) {
            Label("Available Offers (\(viewModel.baskets.count))", systemImage: "leaf.fill")
                .font(.title3.bold())
